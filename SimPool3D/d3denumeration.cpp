...
//       mode can be found.
//-----------------------------------------------------------------------------
bool CD3DEnumeration::FindBestWindowedMode( bool bRequireHAL, bool bRequireREF, HWND hWnd )
{
...
EndWindowedDeviceComboSearch:
	...
	d3dSettings->Windowed_Width = rcWindowClient.right - rcWindowClient.left;
    d3dSettings->Windowed_Height = rcWindowClient.bottom - rcWindowClient.top;
    if (AppUsesDepthBuffer)
...
//       mode can be found.
//-----------------------------------------------------------------------------
bool CD3DEnumeration::FindBestFullscreenMode( bool bRequireHAL, bool bRequireREF )
{
...
EndFullscreenDeviceComboSearch:
    ...
    d3dSettings->pFullscreen_AdapterInfo = pBestAdapterInfo;
    d3dSettings->pFullscreen_DeviceInfo = pBestDeviceInfo;
    d3dSettings->pFullscreen_DeviceCombo = pBestDeviceCombo;
    d3dSettings->IsWindowed = false;
    d3dSettings->Fullscreen_DisplayMode = bestDisplayMode;
    if (AppUsesDepthBuffer)
        d3dSettings->Fullscreen_DepthStencilBufferFormat = *(D3DFORMAT*)pBestDeviceCombo->pDepthStencilFormatList->GetPtr(0);
    d3dSettings->Fullscreen_MultisampleType = *(D3DMULTISAMPLE_TYPE*)pBestDeviceCombo->pMultiSampleTypeList->GetPtr(0);
    d3dSettings->Fullscreen_MultisampleQuality = 0;
    d3dSettings->Fullscreen_VertexProcessingType = *(VertexProcessingType*)pBestDeviceCombo->pVertexProcessingTypeList->GetPtr(0);
    d3dSettings->Fullscreen_PresentInterval = D3DPRESENT_INTERVAL_DEFAULT;
    return true;
}
